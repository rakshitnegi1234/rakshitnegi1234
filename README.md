<!-- Banner & Header Section -->
<p align="center">
  <img src="https://github.com/rakshitnegi1234/rakshitnegi1234/blob/main/Blue%20Yellow%20Futuristic%20Virtual%20Technology%20Blog%20Banner%20(1).png?raw=true" alt="Rakshit Negi Banner" width="100%" />
</p>

<h1 align="center">Hi there! 👋 I'm Rakshit Negi</h1>

<h3 align="center">
  <samp>&gt; Software Engineer | Full Stack Developer | System Design Enthusiast &lt;</samp>
</h3>

<p align="center">
  <em>Architecting scalable web applications, real-time systems, and intelligent AI-powered experiences.</em>
</p>

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=rakshitnegi1234&label=Profile%20Views&color=0ea5e9&style=flat" alt="Profile Views" />
</p>

<p align="center">
  <a href="mailto:rakshitnegi7@gmail.com">
    <img src="https://img.shields.io/badge/Let's_Talk-rakshitnegi7@gmail.com-blue?style=for-the-badge&logo=gmail&logoColor=white" alt="Email" />
  </a>
</p>

---

<!-- About Me Section -->
<table align="center" width="100%" style="border-collapse: collapse;">
  <tr>
    <td width="60%">
      <h2>🚀 Behind the Code</h2>
      <ul>
        <li>🎓 <strong>B.Tech CSE Student</strong> at Jaypee Institute of Information Technology.</li>
        <li>💻 <strong>Full Stack Developer</strong> focused on building clean, impactful, and user-centric products.</li>
        <li>🏗️ <strong>Architecture Focus:</strong> Currently deep-diving into high-availability systems, exploring database horizontal sharding and master-slave replication setups.</li>
        <li>🤖 <strong>AI Integration:</strong> Passionate about bridging the gap between deep learning and modern web applications.</li>
        <li>🎸 <strong>Off-Screen:</strong> When I'm not optimizing databases or debugging, you can find me playing the guitar and blending creativity with logic.</li>
      </ul>
    </td>
    <td width="40%" align="center">
      <img src="https://media.giphy.com/media/qgQUggAC3Pfv687qPC/giphy.gif" alt="Coding GIF" width="280" style="border-radius: 15px; box-shadow: 0 4px 8px rgba(0,0,0,0.2);" />
    </td>
  </tr>
</table>

---

<!-- Tech Stack Section -->
<h2 align="center">🛠️ Tech Arsenal</h2>

<table align="center" width="100%">
  <tr>
    <td align="center" width="20%">
      <strong>💻 Languages</strong><br><br>
      <img src="https://skillicons.dev/icons?i=cpp,js,ts,html,css" alt="Languages" />
    </td>
    <td align="center" width="20%">
      <strong>⚡ Frameworks</strong><br><br>
      <img src="https://skillicons.dev/icons?i=react,nodejs,express,tailwind,firebase" alt="Frameworks" />
    </td>
    <td align="center" width="20%">
      <strong>🗄️ Databases</strong><br><br>
      <img src="https://skillicons.dev/icons?i=mongodb,mysql,postgresql" alt="Databases" style="margin-bottom: 8px;"/><br>
      <!-- Original Logos without text -->
      <img src="https://github.com/neo4j.png" width="48" alt="Neo4j" /> &nbsp;
      <img src="https://github.com/pinecone-io.png" width="48" alt="Pinecone" />
    </td>
    <td align="center" width="20%">
      <strong>☁️ Tools & Cloud</strong><br><br>
      <img src="https://skillicons.dev/icons?i=docker,aws,git,github,vscode" alt="Tools" />
    </td>
    <td align="center" width="20%">
      <strong>🤖 AI, Agents & LLMs</strong><br><br>
      <!-- Original Logos without text -->
      <img src="https://github.com/langchain-ai.png" width="48" alt="LangGraph" style="margin-bottom: 8px;"/><br>
      <img src="https://github.com/deepseek-ai.png" width="48" alt="DeepSeek" /> &nbsp;
      <img src="https://github.com/mistralai.png" width="48" alt="Mistral" />
    </td>
  </tr>
</table>

---

<!-- Resume Projects Section -->
<h2 align="center">💼 Featured Engineering Projects</h2>
<p align="center"><em>A showcase of my best technical work and architectural decisions.</em></p>

> ### 🥇 [Your Top Resume Project Name Here]
> *A brief, punchy one-line description of the project and the core problem it solves.*
> - **Tech Stack:** React • Node.js • MongoDB • AWS
> - **Impact:** What makes this project impressive? (e.g., Handled real-time data flow, optimized database queries, or implemented secure authentication).
> - **Links:** [🔗 Live Demo](#) | [💻 Source Code](#)

<br>

> ### 🥈 [Your Second Resume Project Name Here]
> *A brief, punchy one-line description of the project and the core problem it solves.*
> - **Tech Stack:** TypeScript • Express • PostgreSQL
> - **Impact:** Highlight any complex systems (e.g., implemented efficient data retrieval, replication setups, or complex AI/Gemini integrations).
> - **Links:** [🔗 Live Demo](#) | [💻 Source Code](#)

*(Note: Replace the bracketed text and `#` links with your actual project details!)*

---

<!-- GitHub Stats Section with 30-min Cache Refresh -->
<h2 align="center">📈 GitHub Analytics & Activity</h2>

<!-- Dynamic Activity Wave Graph (Last 31 Days) -->
<p align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=rakshitnegi1234&bg_color=0D1117&color=7aa2f7&line=7aa2f7&point=f7768e&area=true&hide_border=true&theme=tokyonight&cache_seconds=1800" width="100%" alt="GitHub Activity Graph" />
</p>

<!-- GitHub Trophies -->
<p align="center">
  <img src="https://github-profile-trophy.vercel.app/?username=rakshitnegi1234&theme=tokyonight&no-frame=true&no-bg=true&margin-w=15" alt="GitHub Trophies" />
</p>

<!-- Contribution Streak Card -->
<p align="center" style="margin-top: 15px;">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=rakshitnegi1234&theme=tokyonight&hide_border=true&background=0D1117&date_format=j%20M%5B%20Y%5D&ring=7aa2f7&fire=f7768e&currStreakLabel=7aa2f7" alt="GitHub Streak" />
</p>

---

<!-- Connect & Philosophy Section -->
<h2 align="center">🌐 Let's Connect!</h2>

<p align="center">
  <a href="https://linkedin.com/in/rakshit-negi-a57523251" target="_blank">
    <img src="https://skillicons.dev/icons?i=linkedin" width="50" alt="LinkedIn" />
  </a>
  &nbsp;&nbsp;
  <a href="https://instagram.com/sporty_serenades" target="_blank">
    <img src="https://skillicons.dev/icons?i=instagram" width="50" alt="Instagram" />
  </a>
</p>

<br>

<p align="center">
  <samp>
    "Build. Learn. Improve. Repeat."
  </samp>
</p>

<!-- Animated Waving Footer -->
<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0b192c&height=100&section=footer" width="100%" alt="Footer Wave"/>
</p>
